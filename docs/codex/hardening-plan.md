...
9. record owner-supplied conntrack original-destination/443 identity, exact
   ...
   and the one normal reboot at `2026-09-01 13:14:41`: host reachable in about
   40 seconds, perimeter service active/enabled with boot re-application, the
   same 3+1 rule counts, both current containers healthy at `t+5s`, candidate-
   image prefix `sha256:153987840361...`, allowed 200 with valid TLS in 0.13
   seconds, IPv6/443 blocked, port 80 and ACME unaffected, loopback 403, and
   retained `traefik-traefik-1` stopped/exit 0 with `unless-stopped`,
   runtime `ports={}`, and no 80/443 listener;
...
12. classify that one normal reboot as closure of the last point-in-time
   perimeter-persistence element without generalizing to future reboot or
   crash/power-loss recovery, and reconcile the resulting evidence without
   claiming production completeness for any unverified public, monitoring,
   backup or recovery control.
...
normal-reboot survival observation that closes the last point-in-time perimeter-
persistence element. Future reboot after relevant drift and crash/power-loss
recovery require repeat proof; forced renewal, public router rollback, complete
secret lifecycle, monitoring/backup/recovery acceptance, and customer readiness
remain separate gates. An agent cannot grant itself an exception to those
gates.
...
| D-022 | I5's loopback-published topology could not combine ingress with egress denial; I6 replaces it. | Historical I12 evidence verifies the selected internal API/private peer with no API host port/default route, denied API/native egress, and a socketless dual-attached proxy whose default route is ingress. Later owner records add route/TLS/caller evidence and measured IPv4/IPv6 perimeter behavior through Docker-service restart and one normal reboot. | The normal reboot closes the point-in-time perimeter-persistence exit. Future reboot after relevant drift and crash/power-loss recovery require repeat proof; forced renewal, public router rollback, secret lifecycle, monitoring/recovery acceptance, and customer readiness remain open. Async worker remains deferred. |
| D-023 | Activation evidence must not collapse host-network timeout, router HTTP 403, or application HTTP 401. | The measured host-network deny increments exact counters but times out at the caller; Traefik returns plain 403 only when reached; the backend returns its 401 envelope for a wrong key. IPv4 uses conntrack original destination/443 in `DOCKER-USER`; IPv6 docker-proxy bypasses that chain and is blocked in `INPUT`. The historical rehearsal contract still treats `*_rollback_uncertain` as `STOP/UNKNOWN`. | Retain the diagnostic log, repeat proof after future reboot or relevant drift, public router rollback/final-dark, forced renewal, current address ownership, and single-host firewall scope as separate exits. Crash/power-loss recovery is not inferred from the one normal reboot. Do not reopen `REJECT` versus `DROP` absent new evidence, and authorize no caller expansion. |
