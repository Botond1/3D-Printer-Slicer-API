...
| Selected-profile immutability and Orca inheritance | `IMPLEMENTED_FOCUSED; FINAL_EXACT_IMAGE_BUILD_EQUALITY_AND_HTTP_E2E_PASS` | After selection and before bounds/runtime derivation, Prusa profile bytes are bounded-read from canonical real paths and copied to exclusive job scratch. Orca always resolves the bounded, allowlisted versioned repository copy of the v2.3.1 `Custom` machine/process chain, rejects unknown/cyclic/name- or role-mismatched inheritance, removes `inherits`, and snapshots flattened JSON. The Docker build gate requires canonical semantic equality with the exact pinned native parent files. Symlink/non-canonical/non-regular sources and detected size growth fail closed. Bounds, runtime derivation, digest, and native arguments use that snapshot lineage; public metadata keeps the original selected child basename. A parent-only mutation changes the digest. The equality gate and HTTP E2E passed on exact local image `sha256:66697a1ca69e13600a91481bf474d042c0f89b236ccbaf67fcf2dea8824f2c7f` built from code SHA `ed85eec63409b7362fe05c2b99031eeb24b5b9c9`. |
...
The final local image identity and aggregate pass; hosted exact-SHA validation
remains unverified. External production activation is outside repository
