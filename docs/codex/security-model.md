...
## J0 W2/W3 response and slice-principal control delta
...
| --- | --- | --- |
| Effective profile identity | `IMPLEMENTED_FOCUSED_TESTED` | Every successful Prusa/Orca payload requires one lowercase SHA-256 under `profiles.effective_profile_sha256`. Canonical identity covers engine, technology, configured machine and non-request process layers, stable server-added Orca settings including empty `layer_gcode` and `use_relative_e_distances='1'`, and the request-independent native invocation policy; paths, request/job/model identity, request layer height, and request infill are excluded. This is configuration/invocation-policy identity, not an engine-version, filament, final-dimensions, artifact, or customer identity. |
| Selected-profile immutability and Orca inheritance | `IMPLEMENTED_FOCUSED; VALIDATION_IMAGE_BUILD_EQUALITY_AND_HTTP_E2E_PASS; FINAL_IMAGE_ID_PENDING` | After selection and before bounds/runtime derivation, Prusa profile bytes are bounded-read from canonical real paths and copied to exclusive job scratch. Orca always resolves the bounded, allowlisted versioned repository copy of the v2.3.1 `Custom` machine/process chain, rejects unknown/cyclic/name- or role-mismatched inheritance, removes `inherits`, and snapshots flattened JSON. The Docker build gate requires canonical semantic equality with the exact pinned native parent files. Symlink/non-canonical/non-regular sources and detected size growth fail closed. Bounds, runtime derivation, digest, and native arguments use that snapshot lineage; public metadata keeps the original selected child basename. A parent-only mutation changes the digest. The equality gate and corrected HTTP E2E passed on the validation image; a last source-only rebuild may change its final identity, which is not yet recorded. |
...
| OpenAPI runtime-error alignment | `IMPLEMENTED_FOCUSED_TESTED` | The four requested omissions, `FILE_PROCESSING_TIMEOUT`, `INTERNAL_PROCESSING_ERROR`, `ORCA_PROFILE_INCOMPATIBLE`, and `MODEL_OUT_OF_PRINTER_BOUNDS`, are documented without renaming runtime `errorCode`. The adjacent review also places the already-live `MODEL_DIMENSIONS_UNAVAILABLE` only in the general 422 `oneOf` branch and completes the live HTTP 500 enum with `INTERNAL_PROCESSING_ERROR`, `QUEUE_INTERNAL_ERROR`, `UPLOAD_STORAGE_ERROR`, and `INTERNAL_SERVER_ERROR`. The disjoint bounds branch requires both model dimensions and build-volume limits. |
| Native engine version response | `IMPLEMENTED_FOCUSED_AND_VALIDATION_EXACT_IMAGE_TESTED; FINAL_IMAGE_ID_PENDING` | Before listen, both selected executables run bounded `--help`; the initialized version map is published only after both outputs contain exactly one valid engine-specific version plus the expected help sentinel. Failure rejects startup. Requests read the startup map and every success/OpenAPI schema requires `engine_version`. Focused atomic-init/cache/retry/response contracts pass. Exact-image help probes returned exit 0 with 6087-byte Prusa and 5121-byte Orca output; `--version` returned exit 1 for both and is not used. The actual startup module passed in a network-disabled, non-root, read-only exact-image envelope and atomically published Prusa `2.8.1+linux-x64-GTK3-202409181416` and Orca `2.3.1`; corrected HTTP E2E also returned Orca `2.3.1`. The final rebuilt image identity is not yet recorded. |
| Orca placement and orientation ownership | `IMPLEMENTED_FOCUSED_AND_VALIDATION_EXACT_IMAGE_HTTP_E2E_TESTED; FINAL_IMAGE_ID_PENDING` | Orca arguments pass `--arrange 1` and `--orient 0` after request preprocessing and bounds validation. Arrangement translates the already-rotated model onto the build plate, while auto-orient remains disabled and cannot replace the request-owned rotation. The superseded arrangement-disabled HTTP probe retained negative Y after an X90 origin rotation and failed with native status 206 / `Nothing to be sliced`; its earlier translated direct-smoke fixture did not cover this seam. The corrected network-disabled, read-only, healthy-container HTTP E2E passed with pre-request dimensions 20 x 30 x 10 mm, request rotation X90, and final dimensions 20 x 10 x 30 mm. It also returned Orca `2.3.1`, a lowercase SHA-256-shaped digest, and original profile basenames; `x-api-key` was rejected with HTTP 401, and exact cleanup passed. The final rebuilt image identity is not yet recorded. |
...
| Explicit slice-auth mode | `IMPLEMENTED_FOCUSED_TESTED` | `SLICE_SERVICE_AUTH_MODE` defaults to `legacy`, which requires the shared active and forbids principal material/expiry. `migration` requires shared active, both principal actives, and a future `SLICE_SERVICE_LEGACY_MIGRATION_UNTIL` no more than 90 days away. Shared active/previous authorize only while request time is strictly before expiry; at and after expiry they return the stable rejection while principal slots continue, and all six resolved slots are still compared. `principals` requires both principal actives and forbids shared active/previous and expiry; it is the route-activation target. Optional previous slots require their own active. A configured valid `ADMIN_API_KEY` participates in global uniqueness; only its exact authorized substitution for one missing non-slice active avoids duplicate self-registration. Mode, slot, or deadline mismatch fails before listen. External production activation is outside repository evidence and authority. |
| Principal-only dark activation gate | `RUNBOOK_DEFINED; NOT_RUN_REPOSITORY_EVIDENCE` | Before any router action, sanitized resolver readback must prove `principals`, both principal actives, `legacyAccepted=false`, `expiresAt=null`, and absent shared active/previous plus expiry. One private synthetic slice per principal must pass; available retired shared credentials under `x-slicer-api-key` and a correct principal under `x-api-key` must return exact 401 without workspace, queue, or artifact effects. Queue/artifact cleanup must be exact. Missing or inconclusive evidence keeps the route dark. External production activation is outside repository evidence and authority. |
| Public read surfaces | `UNCHANGED` | `GET /health` and `GET /pricing` remain authentication-free. W3 neither broadens protected routes nor treats public liveness/pricing as operational readiness. |
| Filament and material-mass identity | `BLOCKED_OWNER_INPUT_NOT_STARTED` | Filament-profile identity and `material_used_g` are a separate W8 prerequisite. W8 must not start until the owner supplies the required Bambu reference profile fields; neither can be inferred from the effective-profile digest or current response fields. |

The final rebuilt image identity, final aggregate, and exact-SHA hosted
validation remain pending. External production activation is outside repository
evidence and authority. See
...
| Scoped credential compromise or brute force | Critical | Current repository candidate: distinct pricing/artifact/operations slots plus explicit shared/migration/principal-only slice modes; WooCommerce and LeadPilot rotate independently; fixed-digest comparison, protected-route throttling, exact errors, two-restart revocation, and finite migrations remain fail closed | [`service-auth.js`](../../app/config/service-auth.js), [`requireAudience.js`](../../app/middleware/requireAudience.js), route chains | final repository aggregate remains pending; external production activation is outside repository evidence and authority | Complete repository gates; external production activation requires separate authority |
