...
| --- | --- | --- |
| Effective profile identity | `J1_IMPLEMENTED_FOCUSED_TESTED` | Every successful Prusa/Orca payload requires one lowercase SHA-256 under `profiles.effective_profile_sha256`. Canonical identity covers engine, technology, normalized material, configured machine and non-request process layers, selected Orca filament JSON or explicit null, stable server-added Orca settings including empty `layer_gcode` and `use_relative_e_distances='1'`, and the request-independent native invocation policy; Prusa export flags and Orca's ordered machine-process-filament settings precedence are composed from that same policy. Paths, request/job/model identity, request layer height, and request infill are excluded. This is configuration/invocation-policy identity, not an engine-version, final-dimensions, artifact, customer, or live-calibration identity. |
...
| Public read surfaces | `UNCHANGED` | `GET /health`, `GET /ready`, and `GET /pricing` remain authentication-free. `/health` is cheap liveness and `/ready` is minimal readiness. J1C proposes, but does not implement, future native capability state on `/ready`. |
| Repository filament and material-mass identity | `J1_IMPLEMENTED_FOCUSED_TESTED` | Orca PLA/PETG requests select an exact repository filament profile, snapshot it, load native settings in machine-process-filament order, bind normalized material plus filament JSON into the profile digest, and expose the selected basename with actual profile diameter/density. Missing or unsupported material preserves `filament_profile:null`, null metadata, a distinct digest, `material_used_g:null`, `hourly_rate:null`, and `stats.estimated_price_huf:null`; the API calculates no automatic price. OpenAPI requires nullable FDM `material_used_g`; any non-null mass comes only from the slicer's direct G-code marker and is never inferred from length. |
| Strict G-code metric drift | `J1C_FOCUSED_19_OF_19_PASS; LOCAL_AGGREGATE_PASS; CONTAINER_NOT_VERIFIED` | `SLICE_STRICT_GCODE_METRICS` defaults to true and requires positive print-time and filament-length markers. On optional-mass paths, a missing or recognized non-positive grams marker becomes null/manual and is never published as zero or derived from length. Selected-profile Orca still requires positive direct grams within `MAX_MATERIAL_USED_GRAMS`; recognized zero remains `GCODE_FILAMENT_NOT_POSITIVE`, and missing/drifted mass maps to bounded HTTP 500 `SLICE_OUTPUT_UNPARSED`. Explicit false is retained only for bounded drift diagnosis. |
| Live Bambu calibration | `BLOCKED_MISSING_SELF_CONTAINED_VENDOR_INPUT` | The repository P1S/H2D candidates remain generic Marlin profiles. The bounded audit parsed 11/11 supplied JSON files, matched 11/11 declared hashes, and derived P1S 256 x 256 x 250 mm plus H2D 350 x 320 x 325 mm, but 11 include templates, H2D-compatible and 0.1/0.3 BBL processes, vendor filament/parent chains, and material fields are missing; redistribution/license and exact Orca 2.3.1 compatibility are unverified. No vendor/resolver/runtime change was made and the selected-filament Orca incompatibility remains not fixed. |
