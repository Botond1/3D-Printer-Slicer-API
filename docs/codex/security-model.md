...
| Fixture integrity | `MEASUREMENT_A_44_OF_44_PASS; FINAL_B_88_OF_88_PASS; OWNER_MATRIX_VERIFIED_DB42B93` | Normal fixtures require outward non-zero facet normals plus immediate native `prusa-slicer --info` dimension validation. The deliberately zero-normal legal binary STL is a separate regression row: exact J2, exact local B, and the owner production-identical VPS matrix returned HTTP 200 on both engines; schema 2 reports false/null original provenance. It cannot be reported as a normal service defect. |
...
| Exposure and release boundary | `ZERO_CUSTOMER_EXPOSURE; MERGE_AUTHORIZED_NOT_YET_COMPLETE; DEPLOY_NOT_AUTHORIZED` | The plugin has no production deployment/traffic and LeadPilot slicing is not enabled. One branch push, one PR into `main`, and that PR's merge are owner-authorized but not yet claimed complete. Registry/image publication, deploy, production-container, public-route/DNS/allowlist mutation, consumer-repository changes, and customer traffic remain out of scope. |
...
selector, and 37/37 HTTP checks. The owner later ran the complete production-
identical VPS matrix from exact tree
`db42b93b2416ac0b791a45a0eae1233b303cf557` after independently matching all
445 tracked files. The owner build's different image ID is not treated as byte-
identical-image evidence. Its matrix confirmed inclusive P1S/H2D-QUOTE
boundaries on all four selectors, full K2 422 conversion of the former native
500 cases, zero-normal false/null degradation, distinct applied/preserved/
unchanged outcomes, unchanged mass/no-yaw guards, and all three enlarged Prusa
layer profiles. See
...
the later J3B owner production-identical VPS rerun also passed on exact tree
`db42b93`. See
...
| Denial-layer separation | `OWNER_REPORTED_FIREWALL_TIMEOUT; EXTERNAL_403; APPLICATION_401_ENVELOPE_PRESERVED` | The host-network deny increments its exact counter but returns no bytes and is caller-visible as a timeout. A request that reaches Traefik from an unlisted source receives exact HTTP 403 with `Content-Length: 9`, body `Forbidden`, and no `Content-Type`. A recognized source with a wrong key reaches the backend and receives HTTP 401 with the API envelope. Timeout, edge 403, and application 401 are deliberately distinct. |
| IPv4 host-network second layer | `OWNER_REPORTED_CONNTRACK_443_PASS; CALLER_TIMEOUT_MEASURED` | Three `REJECT` variants all incremented the deny counter but produced only a full caller timeout; `REJECT` versus `DROP` must not be reopened absent new evidence. The final rules match conntrack original destination plus original port 443 in IPv4 `DOCKER-USER`; no plain internal/public destination-port match or port-80 match is admitted. Because iptables cannot distinguish Host/SNI, this layer is accepted only while Traefik serves one hostname. |
| IPv6 listener seam | `OWNER_REPORTED_INPUT_443_BLOCK_PASS` | This host has no IPv6 DNAT; `[::]:443` is served through docker-proxy and never traverses `DOCKER-USER`. Before correction an arbitrary IPv6 source completed TLS and received Traefik 403. The network deny therefore belongs in `ip6tables INPUT` for new TCP/443. IPv6 port 80 remains untouched in the no-`AAAA`, IPv4-ACME topology. |
| Perimeter durability | `OWNER_REPORTED_IDEMPOTENT_3_IPV4_1_IPV6; DOCKER_RESTART_PASS; NORMAL_REBOOT_PASS; POINT_IN_TIME_COMPLETE` | Three applications remained exactly three owned IPv4 rules plus one owned IPv6 rule and survived a Docker-service restart. At the owner-observed normal reboot timestamp `2026-09-01 13:14:41`, the host returned in about 40 seconds, `r3d-perimeter.service` was active/enabled and reapplied the same 3+1 policy, and both current service containers were healthy at `t+5s`. The API remained on candidate-image prefix `sha256:153987840361...`; the approved caller returned HTTP 200 with valid TLS in 0.13 seconds; IPv6/443 stayed blocked; port 80 and ACME were unaffected; and loopback returned 403. Retained `traefik-traefik-1` stayed stopped/exit 0 with `unless-stopped`, runtime `ports={}`, and no 80/443 listener. This closes the last open perimeter-persistence element for that exact observed configuration, not future reboot or crash/power-loss behavior. |
| Versioned host artifacts | `REPOSITORY_VERSIONED; PRIVATE_INPUTS_REQUIRED` | The exact commented script, loopback probe, and systemd unit live under `ops/hostinger/perimeter/`. The allowlist and public-IPv4 file paths are mandatory root-private operator inputs; the probe repository default is only `slicer-api.invalid`, and the real hostname is operator input. |
| Authority boundary | `THIS_REPOSITORY_TURN_DOCUMENTATION_ONLY_NO_LIVE_MUTATION` | The owner-supplied completed host operation is recorded, but this repository turn performs no deploy, container replacement, mounted-directory write, router activation, DNS/allowlist/firewall mutation, consumer change, or customer traffic. It does not independently repeat the live record. Future reboot after relevant drift and crash/power-loss recovery require repeat proof; public router rollback, forced renewal, monitoring, and customer readiness remain separate. |
...
| Private source allowlist | `HISTORICAL_J2_CONTRACT; SUPERSEDED_BY_SINGLE_SOURCE_AND_PERIMETER_CORRECTION` | Historical J2 rendering admitted one through four canonical private IPv4 `/32` ranges and assumed a caller-visible host-firewall reset. The current Hostinger correction above accepts exactly one source and supersedes that untested reset assumption with measured IPv4 timeout, Traefik HTTP 403, backend HTTP 401, and the separate IPv6 `INPUT` seam. |
...
| Production topology/promotion | `OWNER_REPORTED_LIVE_ROUTE_AND_PERIMETER_PARTIAL; PERIMETER_PERSISTENCE_POINT_IN_TIME_COMPLETE; FULL_ACCEPTANCE_UNVERIFIED` | At the I9 boundary no VPS or deployed topology proof existed. Historical I12 evidence later verified one exact dark digest, private peer, egress denial, readiness, and socketless proxy. Current owner-supplied records add exact `router_activation=PASS phase=leadpilot-only entries=1`, certificate issuance, approved-source HTTP 200, external unlisted-source HTTP 403, redirect-follow completion on public 443, IPv4 timeout/counters, IPv6 `INPUT` blocking, idempotency, Docker-service restart survival, and one normal-reboot survival observation that closes the point-in-time perimeter-persistence exit. Future reboot after relevant drift and crash/power-loss recovery require repeat proof; forced renewal, public router rollback, monitoring/recovery acceptance, and customer readiness remain unverified. |
...
| Owner-supplied route activation is not full production acceptance. | Critical | I11 completed automatic no-deploy rehearsal at run `32669484893`, and historical I12 verifies one exact dark API digest and corrected proxy topology. Later owner records add the live LeadPilot-only route, issued certificate, caller matrix, measured dual-stack perimeter, idempotency, Docker-service restart survival, and one normal-reboot survival observation that closes the point-in-time perimeter-persistence exit. | **Residual acceptance gate:** future reboot after relevant drift and crash/power-loss recovery require repeat proof; forced renewal, public router rollback, complete secret lifecycle, monitoring/backup/recovery acceptance, and customer readiness remain open. |
...
| A port-wide IPv4 `DOCKER-USER` rule can silently block a later hostname. | High | The owner-observed IPv4 path uses Docker DNAT and `DOCKER-USER`; a 443 rule cannot distinguish HTTP Host or TLS SNI. | **Single-host gate:** re-read the host firewall before mutation and use the rule only while Traefik serves exactly one hostname. A second hostname requires stop/redesign. |
| IPv6 can bypass an IPv4-complete-looking perimeter. | Critical | On this host `[::]:443` uses docker-proxy without IPv6 DNAT, so IPv6 never traverses `DOCKER-USER`. Before the `INPUT` correction an arbitrary IPv6 source completed TLS and received Traefik 403. | **Dual-stack gate:** keep new IPv6/443 blocked in `ip6tables INPUT` while there is no approved IPv6 caller or `AAAA`; leave port 80 untouched. Any IPv6 caller, `AAAA`, listener implementation, or hostname change requires redesign and repeat proof. |
| An allowlist block can be misdiagnosed as a bad application key or dead API. | High | Current evidence separates a caller-visible host-firewall timeout with incremented counters, router HTTP 403 without `Content-Type`, and application HTTP 401 with its envelope. The owner reports the consumer was told to treat timeout as the third outcome, but consumer code is outside this repository. | **Residual rehearsal gate:** retain the diagnostic log, public router rollback, and target-filesystem durability. Repeat the matrix after any future reboot or relevant drift; crash/power-loss recovery is not inferred from the single normal reboot. Never collapse timeout, edge 403, or application 401. |
