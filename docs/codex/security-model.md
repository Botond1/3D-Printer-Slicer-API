...
| Private source allowlist | `HISTORICAL_J2_CONTRACT; SUPERSEDED_BY_SINGLE_SOURCE_CORRECTION` | Historical J2 rendering admitted one through four canonical private IPv4 `/32` ranges. The current Hostinger correction above removes expansion and accepts exactly one source. Router denial is HTTP 403, host-firewall denial is TCP reset plus private `J2_ALLOWLIST_DENY`, and application principal failure remains HTTP 401. |
