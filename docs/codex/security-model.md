...
## J1 calibration harvest over the J0 W2/W3 control delta
...
| Strict G-code metric drift | `J1_IMPLEMENTED_FOCUSED_TESTED` | `SLICE_STRICT_GCODE_METRICS` defaults to true and requires positive print-time and filament-length markers. The current Prusa FDM profile emits no direct mass marker, so its successful response has null mass/rate/price for manual pricing, never zero or a length-derived mass. Orca with a selected filament profile additionally requires positive direct grams within `MAX_MATERIAL_USED_GRAMS`; missing or drifted mass maps to bounded HTTP 500 `SLICE_OUTPUT_UNPARSED`. Profile-less Orca remains null/manual. Explicit false is retained only for bounded drift diagnosis. |
| Live Bambu calibration | `BLOCKED_OWNER_INPUT` | The retained P1S and new H2D candidates identify as generic Marlin profiles, not verified native Bambu profiles. Real machine/process references, approved material/spool references, ten owner-selected calibration models, and owner-approved acceptance thresholds are required before W8 live calibration. J1 repository/focused evidence does not prove a printer, calibration, deployment, public route, or production traffic. |
