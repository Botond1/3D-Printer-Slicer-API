...
| Perimeter durability | `OWNER_REPORTED_IDEMPOTENT_3_IPV4_1_IPV6; DOCKER_RESTART_PASS; REAL_REBOOT_NOT_VERIFIED` | Three applications remained exactly three owned IPv4 rules plus one owned IPv6 rule. Docker-service restart survival, allowed HTTP 200 in about 0.1 seconds, blocked IPv6 no-response, port-80 reachability, and loopback Traefik 403 were reported. State after a real host reboot remains unverified. |
...
| Authority boundary | `THIS_REPOSITORY_TURN_DOCUMENTATION_ONLY_NO_LIVE_MUTATION` | The owner-supplied completed host operation is recorded, but this repository turn performs no deploy, container replacement, mounted-directory write, router activation, DNS/allowlist/firewall mutation, consumer change, or customer traffic. It does not independently repeat the live record; real reboot, public router rollback, forced renewal, monitoring, and customer readiness remain separate. |
