...
### Added
- **[contract]** The opt-in catalogue v3 (`GET /profiles?contract=material-v1`) carries `build_volume_limits_mm.alternative_footprints_inclusive_mm` on every row: the X/Y footprints the slice endpoint admits in addition to `largest_passing_dimensions_inclusive_mm`, up to the same Z, through the API-owned placement. The Bambu P1S rows publish `[{ "x": 238, "y": 256 }]` (the bed excludes an `18 x 28 mm` corner, so admission is `256 x 228` above it or `238 x 256` beside it, as the envelope confirmation runner measured and `bambu-placement.js` enforces); every other row publishes `[]`. The catalogue validator requires at most four exact `{x, y}` pairs, only on Bambu rows (placement admission), each inside the declared bed and extending beyond the triple on exactly one of X and Y, one bed shape per machine across its presets, and a unit test binds every published footprint to the real placement (`+0.1 mm` on either axis is refused). A consumer that checked only the triple refused P1S parts the endpoint had accepted: the R3D plugin moved a `179 x 233.8 x 84.5 mm` customer pose to the H2D on 2026-09-22.

### Unchanged
- Catalogue v2 (`GET /profiles`) is byte-identical: the field is stripped from v2 rows exactly like `material_profiles`, so the v2 `catalogue_sha256` does not move. `machine_resolutions` and `fleet_resolutions` keep the triple on both contracts. Admission, placement, the slice responses and the Bambu `measurement_generation` are unchanged (`profiles.js` and the placement modules are not touched; the publication reads `app/config/constants.js`).

## v3.6.0 (2026-09-15)

### Changed
- **[contract]** `POST /bambu/slice` admits ordinary real-world meshes and describes them instead of refusing them. Measured on the owner's own customer files (2 451 distinct models, 2026-09-15): the 3.5.0 admission refused four of five real STLs — zero-area or duplicated triangles (`degenerate_faces`, the reference Benchy included), a handful of open edges (`open_mesh`), several separate shells in one file (`ambiguous_scope`: a key ring, a llama) — and every Bambu Studio project 3MF (135 of 228: their geometry lives in production-extension parts under `3D/Objects/`, which the old scope check called unloadable). `inspect_mesh.py` (schema `r3d-mesh-inspection-v2`) now drops degenerate and duplicate triangles from its analysis copy, admits open meshes and several shells, and reports `watertight`, `winding_consistent`, `component_count`, `closed_component_count`, `open_edge_count`, `dropped_degenerate_faces`, `dropped_duplicate_faces` and a `volume_source` of `validated_triangle_mesh` (closed, consistently wound) or `signed_volume_estimate` (the divergence integral of a winding-fixed copy; Benchy's 29 open edges still give its 15.5 cm3). Only geometry that cannot print is refused: nothing loads, no finite coordinate, every triangle degenerate, a zero extent or no enclosed volume (HTTP 400 `INVALID_SOURCE_GEOMETRY`, reworded). The submitted geometry is still sliced as-is: nothing is repaired or exported from the analysis. Measured in an isolated container on the production image: Benchy 3613 s / 15.11 g, the 8-shell key ring 1911 s / 12.4 g, the 894-fragment `bottom-v1` 1005 s / 4.8 g, the 5-shell llama 4149 s / 22.95 g — the native slicer produced verified single-part toolpaths for all of them.
- **[contract]** `technical_receipt.geometry` carries the description above (booleans and counts instead of the `true`/`1` literals), `technical_receipt.scope.object_count` / `instance_count` carry the 3MF build's own counts (an STL/OBJ stays `1`/`1`), and an open mesh adds the warning `GEOMETRY_NOT_WATERTIGHT` (`source: geometry`; `warnings` may now hold three entries). `scope.kind` stays `single_part`: one manufacturing unit, one plate, one filament, quantity 1 — everything the file holds prints together. Strict readers update the schema (`app/docs/technical-receipt-openapi.js`).
- **[contract]** 3MF is flattened by the API's own walk of the build (`mesh2stl.py`, 3MF core plus the production extension's external model parts): every build item, through every component with its transform, down to the mesh objects, with the root `unit` applied. A generic loader miscounted instanced parts on real files (nine copies of a nine-item plate, three of a six-item one), so it is no longer used for 3MF. The Bambu scope helper (`inspect_mesh.py --3mf-scope`) rides the same walk and reports the build's counts. Still refused as 422 `AMBIGUOUS_MANUFACTURING_SCOPE`: a build the walk cannot satisfy (missing part or object, unbounded nesting) and a slicer project whose `Metadata/model_settings.config` marks modifier, negative or support-blocker/enforcer parts — those are not the printed geometry their meshes describe. Entity declarations stay refused; external references outside the archive were never followed and still are not.
- The Bambu `measurement_generation` changes with this release (`inspect_mesh.py` and `mesh2stl.py` are part of it); consumers re-read the catalogue before pinning new requests.
