...
test('v2 builder emits a deeply frozen bounded exact-key publication record', () => {
    const evidence = buildPublicationEvidence(validInput());
    assert.equal(validatePublicationEvidence(evidence), null);
    assert.equal(evidence.schema_version, 'i8-s3a-signed-candidate-provenance-v2');
...
test('v2 builder rejects invalid inputs and severs caller-owned mutable state', () => {
...
test('source, workflow, build, repository, tag, and digest identities fail closed', async (t) => {
    const mutations = [
        ...
        ['source ref', (x) => { x.source.ref = 'refs/heads/main'; }],
...
test('both signed digest-bound attestations require three verification paths', async (t) => {
    const mutations = [
        ...
        ['wrong source ref', (x) => {
            x.attestations.provenance.source_ref = 'refs/heads/main';
        }],
