...
const {
    ...
    buildFromRepository,
    writeEvidence
} = require('../../../scripts/i11-write-publication-evidence');
...
function createFixture() {
    const runnerTemp = fs.mkdtempSync(path.join(os.tmpdir(), 'i11-publication-'));
    ...
    fs.writeFileSync(
        path.join(evidenceRoot, 'i11-publication-draft.json'),
...
function rewriteDraft(fixture, mutate) {
    const target = path.join(fixture.evidenceRoot, 'i11-publication-draft.json');
...
test('writer rejects a failed or missing gate despite a successful-looking draft', (t) => {
    for (const outcome of REQUIRED_OUTCOMES) {
        ...
        assert.throws(
            ...
            (error) => error.code === 'i11_evidence_gate_outcome_failure',
            outcome
        );
    }
});
...
test('writer recomputes lock, SBOM, Grype hashes and scanner findings', (t) => {
    ...
    for (const [name, mutate] of cases) {
        ...
        assert.throws(
            () => buildFromRepository(fixture.env),
            (error) => error.code === 'i11_evidence_computed_input_mismatch',
            name
        );
    }
});
...
test('writer creates one bounded exclusive output and rejects overwrite', (t) => {
    ...
    assert.throws(
        ...
        (error) => error.code === 'i11_evidence_output_boundary_failure'
    );
});
...
test('writer rejects evidence roots outside the exact runner temp subdirectory', (t) => {
    ...
    assert.throws(
        () => buildFromRepository(fixture.env),
        (error) => error.code === 'i11_evidence_root_mismatch'
    );
});
