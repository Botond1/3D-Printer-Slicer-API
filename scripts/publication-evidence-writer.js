...
const WRITER_PROFILE_KEYS = Object.freeze([
    'contract', 'inputFile', 'outputFile', 'errorPrefix', 'successMarker'
]);

function createPublicationEvidenceWriter(inputProfile) {
    const inputKeys = inputProfile && typeof inputProfile === 'object'
        && !Array.isArray(inputProfile) ? Object.keys(inputProfile).sort() : [];
    if (JSON.stringify(inputKeys) !== JSON.stringify([...WRITER_PROFILE_KEYS].sort())
        || !inputProfile.contract || typeof inputProfile.contract !== 'object'
        || typeof inputProfile.inputFile !== 'string'
        || path.basename(inputProfile.inputFile) !== inputProfile.inputFile
        || typeof inputProfile.outputFile !== 'string'
        || path.basename(inputProfile.outputFile) !== inputProfile.outputFile
        || inputProfile.inputFile === inputProfile.outputFile
        || typeof inputProfile.errorPrefix !== 'string'
        || !/^[a-z][a-z0-9]{0,15}$/.test(inputProfile.errorPrefix)
        || typeof inputProfile.successMarker !== 'string'
        || !/^[a-z][a-z0-9_]{1,79}=PASS$/.test(inputProfile.successMarker)) {
        throw new TypeError('publication_evidence_writer_profile_invalid');
    }
    const {
    ...
    } = inputProfile.contract;
    if (typeof buildPublicationEvidence !== 'function'
        || typeof validatePublicationEvidence !== 'function') {
        throw new TypeError('publication_evidence_writer_contract_invalid');
    }
    const INPUT_FILE = inputProfile.inputFile;
    const OUTPUT_FILE = inputProfile.outputFile;
    const errorPrefix = inputProfile.errorPrefix;
    const successMarker = inputProfile.successMarker;
    const errorCode = (suffix) => `${errorPrefix}_${suffix}`;
...
function regularContainedFile(root, name, limit) {
    ...
    try {
        ...
        if (!details.isFile() || details.isSymbolicLink() || details.size <= 0
            ...
            fail(errorCode('evidence_input_boundary_failure'));
        }
    } catch (error) {
        if (error?.code === errorCode('evidence_input_boundary_failure')) throw error;
        fail(errorCode('evidence_input_boundary_failure'));
    }
    return target;
}
...
function evidenceRootFromEnvironment(environment) {
    if (!environment.RUNNER_TEMP || !environment.EVIDENCE_SUBDIR
        ...
        fail(errorCode('evidence_root_mismatch'));
    }
...
function expectedFromEnvironment(environment, sbomPath, grypePath) {
    if (environment.GITHUB_REPOSITORY !== EXACT_REPOSITORY_SLUG
        ...
        fail(errorCode('evidence_hosted_identity_mismatch'));
    }
    return {
...
function verifyComputedInputs(evidence, inputs, sbomPath, grypePath) {
    ...
    if (evidence.build_inputs.dockerfile_sha256 !== sha256(inputs.dockerfile)
        ...
        fail(errorCode('evidence_computed_input_mismatch'));
    }
}
...
function writeEvidence(environment = process.env) {
    ...
    if (fs.existsSync(target)) fail(errorCode('evidence_output_boundary_failure'));
    const serialized = `${JSON.stringify(evidence, null, 2)}\n`;
    if (Buffer.byteLength(serialized, 'utf8') > MAX_EVIDENCE_BYTES) {
        fail(errorCode('evidence_output_boundary_failure'));
    }
    try {
        ...
        if (error?.code === errorCode('evidence_output_boundary_failure')) throw error;
        fail(errorCode('evidence_output_boundary_failure'));
    }
    return target;
}
...
function main() {
    try {
        ...
        const code = /^[a-z0-9_]{1,80}$/.test(error?.code || error?.message || '')
            ? (error.code || error.message) : errorCode('evidence_generation_failure');
        console.error(code);
        process.exitCode = 2;
    }
}
...
module.exports = Object.freeze({createPublicationEvidenceWriter});
