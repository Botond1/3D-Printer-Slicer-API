...
const CONTAINER_SCRIPT = [
    ...
    "    const calibrationProfile = JSON.parse(fs.readFileSync(runtimeProcessProfile, 'utf8'));",
    "    if (!calibrationProfile || typeof calibrationProfile !== 'object' || Array.isArray(calibrationProfile)) {",
    "        emit({ ok: false, phase: 'profile', code: 'CALIBRATION_PROFILE_INVALID' });",
    '        return;',
    '    }',
    "    calibrationProfile.enable_support = '0';",
    "    fs.writeFileSync(runtimeProcessProfile, JSON.stringify(calibrationProfile, null, 4), {",
    "        encoding: 'utf8', mode: 0o600",
    '    });',
    "    const supportProof = JSON.parse(fs.readFileSync(runtimeProcessProfile, 'utf8'));",
    "    if (supportProof.enable_support !== '0') {",
    "        emit({ ok: false, phase: 'profile', code: 'CALIBRATION_SUPPORT_NOT_DISABLED' });",
    '        return;',
    '    }',
    ...
    "        'FDM', runtimeProcessProfile, desiredOutput, job.infill, 'orca',",
    '        snapshots.orcaMachineConfigFile, snapshots.orcaFilamentConfigFile',
    '    );',
    '    const started = Date.now();',
    '    const sliced = runOrca([...slicerArgs, job.input], job.timeoutMs);',
