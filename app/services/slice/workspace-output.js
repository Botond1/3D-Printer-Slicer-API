...
async function copyOwnedFileToHandle(sourcePath, sourceIdentity, targetHandle) {
...
class OutputCandidateRegistry {
    ...
    async register(originalName, technology) {
        const candidate = path.join(this.outputRoot, buildOutputFilename(originalName, technology));
        ...
        this.candidates.set(candidate, { promoted: false, handle: null, identity: null });
        return candidate;
    }
    ...
    async promote(candidate, source) {
        ...
        await copyOwnedFileToHandle(sourcePath, fileIdentity(sourceStat), record.handle);
